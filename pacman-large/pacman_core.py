    ...
    def FollowNextPathWay(self):
        ...
        if not self.currentPath == False:
            ...
            if len(self.currentPath) > 0:
                ...
                if not self.state == 3:
                    # chase pacman again
                    new_path = path.FindPath((self.nearestRow, self.nearestCol),
                                             (player.nearestRow, player.nearestCol))
                    if new_path and len(new_path) > 0:
                        self.currentPath = new_path
                        self.FollowNextPathWay()
                    else:
                        # 找不到路径（比如地图快通关），就暂停一帧，不继续递归
                        self.currentPath = ""
                        self.velX = 0
                        self.velY = 0
                else:
                    # state == 3, return to ghost box
                    self.state = 1
                    self.speed = self.speed / 4

                    # 给幽灵一个随机含有豆子的目标
                    for _ in range(100):  # 最多尝试 100 次避免死循环
                        randRow = random.randint(1, thisLevel.lvlHeight - 2)
                        randCol = random.randint(1, thisLevel.lvlWidth - 2)
                        if thisLevel.GetMapTile((randRow, randCol)) == tileID['pellet']:
...
class pacman:
    ...
    def __init__(self):
        ...
        # 加速初始化
        self.shadow_trail = []  # 每帧记录位置
        self.speed_boost_timer = 0  # 加速倒计时（帧）
        self.score_checkpoint = 0  # 上一次加速触发的分数点
        self.last_score_when_boost = 0  # 上一次加速结束后的得分
        self.boost_ready = True  # 是否可以触发新一轮加速

        # 隐身
        self.invisible_timer = 0  # 隐身时间（帧）
        self.score_invis_checkpoint = 0  # 上次触发隐身的得分倍数
    ...
    def Move(self):
        ...
        # 每到一个新的200分，触发隐身状态
        # 隐身结束时，更新基准分并允许再次隐身
        if self.invisible_timer == 0 and not self.invis_ready:
            self.last_score_when_invis = thisGame.score
            self.invis_ready = True
        ...
        # 检查是否需要触发新的加速
        # 每帧检查是否加速结束
        if self.speed_boost_timer == 0 and not self.boost_ready:
            # 记录加速结束时的得分基准线
            self.last_score_when_boost = thisGame.score
        ...
        if self.speed_boost_timer > 0:
            self.shadow_trail.append((self.x, self.y))
            if len(self.shadow_trail) > 5:
                self.shadow_trail.pop(0)
        else:
        ...
        if not thisLevel.CheckIfHitWall((self.x + self.velX, self.y + self.velY), (self.nearestRow, self.nearestCol)):
            ...
            for i in range(0, 4, 1):
                if thisLevel.CheckIfHit((self.x, self.y), (ghosts[i].x, ghosts[i].y), TILE_WIDTH / 2):
                    ghost_state = ghosts[i].state

                    if ghost_state == 1:
                        if self.invisible_timer > 0:
                            continue  # 忽略碰撞，因为幽灵正常但我隐身
                        else:
                            thisGame.SetMode(2)  # 被吃

                    elif ghost_state == 2:
                        # 幽灵虚弱，无论是否隐身都能吃
                        thisGame.AddToScore(thisGame.ghostValue)
                        thisGame.ghostValue *= 2
                        snd_eatgh.play()
    ...
    def Draw(self):
        ...
        # 当前动画帧图像
        pac_img = self.anim_pacmanCurrent[self.animFrame].copy()
        ...
        if self.speed_boost_timer > 0:
            for i, (sx, sy) in enumerate(self.shadow_trail):
                shadow = self.anim_pacmanCurrent[self.animFrame].copy()
                shadow.set_alpha(50 - i * 10)  # 越后面越淡
                screen.blit(shadow, (sx - thisGame.screenPixelPos[0], sy - thisGame.screenPixelPos[1]))
        ...
        # 绘制吃豆人
        screen.blit(pac_img, (self.x - thisGame.screenPixelPos[0], self.y - thisGame.screenPixelPos[1]))
